...
public class ReadOnlyGroupManager extends HibernateDaoSupport{
	...
	private boolean isJobsAndGroupsChangedIgnoreContent(){
		...
		changedJobs=getHibernateTemplate().execute(session -> {
            Query query=session.createQuery("select id,groupId from com.taobao.zeus.store.mysql.persistence.JobTask where gmt_modified>:gmt_modified");
            query.setParameter("gmt_modified", ignoreContentJobJudge.lastModified);
            List<Object[]> list=query.list();
            List<JobDescriptor> result=new ArrayList<>();
            for(Object[] o:list){
				JobDescriptor jd=new JobDescriptor();
                jd.setJobId(String.valueOf(o[0]));
	...
	private boolean isAllJobsNotChangeParent(GroupBean gb,List<JobDescriptor> list){
		...
		for(JobDescriptor jd:list){
			JobBean bean=allJobs.get(jd.getJobId());
			if(bean==null){
				DebugInfoLog.info("isAllJobsNotChangeParent job id="+ jd.getJobId()+" has changed");
				return false;
			}
...
//	}
	...
	public synchronized GroupBean getGlobeGroupBean() {
		...
		globe=new ReadOnlyGroupManagerAssembly(groupManager, jobManager).getGlobeGroupBean();
		return globe;
	}
	...
	public synchronized GroupBean getGlobeGroupBeanForTreeDisplay(boolean copy){
		if(ignoreGlobe==null || isJobsAndGroupsChangedIgnoreContent()  ){
			ignoreGlobe=new ReadOnlyGroupManagerAssembly(groupManager,jobManager).getGlobeGroupBean();
		}
		if(copy){
	...
	private class CopyGroupManagerAssembly extends ReadOnlyGroupManagerAssembly {
		private GroupBean globe;
		CopyGroupManagerAssembly(GroupBean globe) {
			super(null, null);
			this.globe=globe;
		}
		@Override
	...
	private class ReadOnlyGroupManagerAssembly implements GroupManager, JobManager {
		private GroupManager groupManager;
		private JobManager jobManager;
		ReadOnlyGroupManagerAssembly(GroupManager gm, JobManager jm){
		...
		public JobStatus getJobStatus(String jobId) {
			return new ReadOnlyJobStatus(jobManager.getJobStatus(jobId));
		}
		...
		public JobBean getUpstreamJobBean(String jobId) {
			return GroupManagerTool.getUpstreamJobBean(jobId, this,this);
		}
		@Override
	...
	public class ReadOnlyJobDescriptor extends JobDescriptor{
		...
		@Override
		public JobDescriptor.JobScheduleType getScheduleType() {
		...
		@Override
		public void setJobType(JobDescriptor.JobRunType type) {
			throw new UnsupportedOperationException();
		}
		@Override
		public void setName(String name) {
		...
		@Override
		public void setScheduleType(JobDescriptor.JobScheduleType type) {
			throw new UnsupportedOperationException();
		}
		@Override
