...
public class WorkerBeCancel {
	...
	Future<Response> cancelManual(final WorkerContext context,final Request req,final String historyId){
		...
		if(!context.getManualRunnings().containsKey(historyId)){
			return context.getThreadPool().submit(() -> Response.newBuilder().setRid(req.getRid()).setOperate(Operate.Cancel).setStatus(Status.ERROR)
	...
	Future<Response> cancelSchedule(final WorkerContext context,final Request req,final String historyId){
		// 查找该job是否在运行中，如果不在，响应ERROR
		// 如果在，执行取消操作，等待操作结束后，响应OK
		JobHistory history=context.getJobHistoryManager().findJobHistory(historyId);
		final String jobId=history.getJobId();
	...
	Future<Response> cancelDebug(final WorkerContext context,final Request req,final String debugId){
		...
		if(!context.getDebugRunnings().containsKey(debugId)){
			future= context.getThreadPool().submit(() -> Response.newBuilder().setRid(req.getRid()).setOperate(Operate.Cancel).setStatus(Status.ERROR)
			.setErrorText("运行任务中查无此任务").build());
			...
			future= context.getThreadPool().submit(() -> {
