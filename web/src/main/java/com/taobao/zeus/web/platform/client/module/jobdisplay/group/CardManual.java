...
public class CardManual extends CenterTemplate implements Refreshable<GroupModel>{
	...
	public CardManual(final GroupPresenter presenter){
		...
		ColumnConfig<JobHistoryModel,Date> startTime=new ColumnConfig<>(prop.startTime(),70,"开始时间");
		...
		ColumnConfig<JobHistoryModel,String> triggerType=new ColumnConfig<>(prop.triggerType(),50,"触发类型");
		...
		ColumnConfig<JobHistoryModel,String> illustrate=new ColumnConfig<>(prop.illustrate(),80,"说明");
		...
		ColumnConfig<JobHistoryModel,JobHistoryModel> operate=new ColumnConfig<>(new ValueProvider<JobHistoryModel, JobHistoryModel>() {
